...
<body>
            ...
            <li>
                <button onclick="toggleSubMenu(this)" class="dropdown-btn rotate">
                    ...
                    <ul class="sub-menu show">
                        <div>
                          <li class="activate"><a href="indigency.php" class="size" >Certificate of Indigency</a></li>
    ...
    <main>
        ...
        <div class="center">
            <h1><code>Welcome to the Barangay Administrative Portal of</code><br>BARANGAY CERTIFICATE OF INDIGENCY</h1>
        </div>  
        <div class="circle">
            <img id="st" src="../img/logs.jpg" alt="">  
          </div>
        ...
        <div class="add">
    <p>LIST OF REQUESTED INDIGENCY</p>
        ...
        <table style="width: 100%;" id="residentTable">
            ...
            <tbody>
                ...
                try {
                    ...
                    $stmt = $pdo->query("SELECT id, CONCAT(first_name, ' ', middle_name, ' ', last_name) AS name, age, date_needed, purpose, tracking_number FROM residents");

                    // Loop through each record and display it in the table
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        echo "<tr>";
                        echo "<td><input type='text' readonly value='{$row['id']}'></td>";
                        echo "<td class='name'><input type='text' readonly value='{$row['name']}'></td>"; 
                        ...
                        echo "<td id='action'><a href='indicertif.php?id={$row['id']}'><button>Extract</button></a></td>";
                        echo "</tr>";
                    }
...
document.getElementById('searchInput').addEventListener('input', function () {
    ...
    rows.forEach(row => {
        // Get the input inside the name column (class 'name')
        const nameInput = row.querySelector('.name input'); 

        if (nameInput) {
            const name = nameInput.value.toLowerCase(); // Convert the name to lowercase for case-insensitive comparison
